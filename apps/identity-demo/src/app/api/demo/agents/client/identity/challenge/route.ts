 ...
 * Verifies the caller's ownership VC and returns the client agent's own
 * ownership VC JWT so the caller can confirm the client agent's identity.
 *
...
export async function POST(req: NextRequest) {
	...
	if (!session.clientOwnershipVC) {
		return NextResponse.json(
			{ error: "Client ownership VC not yet issued." },
			{ status: 400 }
		);
	}
	...
	return NextResponse.json({
		verified: true,
		vcJwt: session.clientOwnershipVC.jwt,
	});
}
